...
const CREDENTIALS_FILE = 'service_account_credentials.json'; // Update with the path to your service account key
const JSON_FILE_PATH = __dirname + '/example.json';
const FOLDER_ID = '1sywyx02h50UJnEPJ3gWQMXpS9mSKai4V'; // Update with the target folder ID in Google Drive
const EMAIL_TO_ADD_PERMISSION = 'solangi.66.izhar@gmail.com'; // Update with the desired email

async function addPermissionToFile(fileId, email, auth) {
    const drive = google.drive({ version: 'v3', auth });

    const permission = {
        type: 'user',
        role: 'reader', // Adjust the role as needed (e.g., 'writer', 'owner')
        emailAddress: email
    };
    ...
    try {
        ...
        console.log('Permission added for email:', email, 'Permission ID:', response.data.id);
    } catch (error) {
        console.error('Error adding permission:', error.message);
    }
}

async function createHTMLWithTableAndUpload() {
    // Load the service account key
    const serviceAccountKey = JSON.parse(fs.readFileSync(CREDENTIALS_FILE));

    // Set up the OAuth2 client with the service account key
    ...
    const title = jsonData.data.content_template.page_title;
    const wordCount = jsonData.data.content_template.min_word_count;
    const h1_tag = jsonData.data.content_template.h1_tag;
    const content = jsonData.data.content_template.opening_content;
    
    const description = jsonData.data.content_template.meta_description;
    // get the keyword and search_volume from primary_keywords  
    const primaryKeywords = jsonData.data.primary_keywords;

    const keywordSearchVolumeValues = primaryKeywords.map(keywordObj => {
      const keyword = keywordObj.keyword;
      const searchVolumeValues = keywordObj.search_volume;
    
      return { keyword, searchVolumeValues };
    });


    // get the keyword and search_volume from secondry_keywords  
    const secondryKeywords = jsonData.data.secondary_keywords;

    const s_keywordSearchVolumeValues = secondryKeywords.map(keywordObj => {
      const keyword = keywordObj.keyword;
      const searchVolumeValues = keywordObj.search_volume;
    ...
    const links = primaryKeywords.reduce((acc, keyword) => {
        const urls = keyword.urls || [];
        const linksForKeyword = urls.map(urlObj => urlObj.link);
        return acc.concat(linksForKeyword);
      }, []);

      const s_links = secondryKeywords.reduce((acc, keyword) => {
        const urls = keyword.urls || [];
        const linksForKeyword = urls.map(urlObj => urlObj.link);
        return acc.concat(linksForKeyword);
      }, []);
...
const s_linkRows = links.map(link => `
      
        ${s_links}<br/>
    
`).join('');

const h2Tags = jsonData.data.content_template.h2_tags || [];
const h2TagsString = h2Tags.join(' ');

const h3Tags = jsonData.data.content_template.h2_tags || [];
const h3TagsString = h2Tags.join(' ');
const h4Tags = jsonData.data.content_template.h2_tags || [];
const h4TagsString = h2Tags.join(' ');
const h5Tags = jsonData.data.content_template.h2_tags || [];
const h5TagsString = h2Tags.join(' ');
const h6Tags = jsonData.data.content_template.h2_tags || [];
const h6TagsString = h2Tags.join(' ');

const breadcrumbs = jsonData.breadcrumbs || [];
const breadcrumbsString = h2Tags.join(' ');
const _final_url = jsonData.data.final_url;

const category = jsonData.data.category;

const internal_links = jsonData.data.internal_links;

const internalLinks = internal_links ? internal_links.map(keywordObj => {
    const uri_links = keywordObj.target_url;
    const text = keywordObj.anchor_text;

    return { uri_links, text };
}) : [];

  const schema = jsonData.data.content_template.schema_markups;
...
<body class="c44 c59 doc-content">
    ...
    <table class="c22">
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c28"><span class="c5">${topic}</span></p>
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                ...
                <p class="c27 c32"><span class="c5"></span></p><a id="t.c2ab6cb3994b667c9584baa154118e32508a8920"></a><a
                    id="t.1"></a>
                <table class="c22" id="Primary-keywords">
                    ...
                    ${keywordSearchVolumeValues.map(({ keyword, searchVolumeValues }) => `
                    <tr class="c14">
                        <td class="c25" colspan="1" rowspan="1">
                            <p class="c28 c32"><span class="c3">${keyword}</span></p>
                        </td>
                        <td class="c25" colspan="1" rowspan="1">
                            <p class="c28 c32"><span class="c3">${searchVolumeValues}</span></p>
                        </td>
                    </tr>`).join('')}
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                ...
                <table class="c22">
                    ...
                    ${s_keywordSearchVolumeValues.map(({ keyword, searchVolumeValues }) => `
                    <tr class="c14">
                        <td class="c25" colspan="1" rowspan="1">
                            <p class="c28 c32"><span class="c3">${keyword}</span></p>
                        </td>
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c5">${title}</span></p>
                <p class="c27 c32"><span class="c3"></span></p>
            </td>
        </tr>
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c8"><span class="c18">${description}</span></p>
            </td>
        </tr>
        <tr class="c14">
            <td class="c11" colspan="1" rowspan="1">
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c5">${wordCount}</span></p>
            </td>
        </tr>
    ...
    <table class="c22">
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c5">${h1_tag}</span></p>
            </td>
        </tr>
        <tr class="c14">
            <td class="c11" colspan="1" rowspan="1">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c5">${content}</span></p>
            </td>
        </tr>
        <tr class="c14">
            <td class="c11" colspan="1" rowspan="1">
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c18 c57">
                ${h3TagsString}
                </span></p>
            </td>
        </tr>
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c57 c18">
                ${h4TagsString}
                </span></p>
            </td>
        </tr>
    ...
    <table class="c22">
        <thead>
            <tr class="c14">
                ...
                <td class="c20" colspan="1" rowspan="1">
                    <p class="c8"><span class="c18">${_final_url}</span></p>
                </td>
        <tbody></tbody>
        </tr>
        <tr class="c14">
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c49 c63 c18 c65">${category}</span></p>
            </td>
        </tr>
        <tr class="c14">
            <td class="c11" colspan="1" rowspan="1">
            ...
            <td class="c20" colspan="1" rowspan="1">
                ...
                <table class="c22">
                    ...
                    ${internalLinks.map(({ uri_links, text }) => `
            <tr class="c14">
                <td class="c25" colspan="1" rowspan="1">
                    <p class="c16"><span class="c3">${uri_links}</span></p>
                </td>
                <td class="c25" colspan="1" rowspan="1">
                    <p class="c16"><span class="c3">${text}</span></p>
                </td>
            </tr>
        `).join('')}
        ...
        <tr class="c14">
            ...
            <td class="c20" colspan="1" rowspan="1">
                <p class="c27"><span class="c5">${schema}</span>
                </p>
            </td>
        </tr>
        </thead>
    ...
    fs.writeFileSync('table.html', htmlContent, 'utf-8');
    ...
    const media = {
        mimeType: 'text/html',
        body: fs.createReadStream('table.html')
    };
    ...
    drive.files.create({
        ...
        if (err) {
            ...
            console.log('File created and uploaded successfully. File ID:', file.data.id);
            // Add permission for the desired email
            await addPermissionToFile(file.data.id, EMAIL_TO_ADD_PERMISSION, auth);
        }
        // Delete the temporary HTML file
        fs.unlinkSync('table.html');
    });
}

// Call the function to create HTML with a table and upload
createHTMLWithTableAndUpload().catch(console.error);
